 ...
 * @author dell
 */
public class DelChqReqServ extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        if(s!=null)
         ...
         for(int i=0;i<s.length;i++)
         {
             ...
             stmt.executeUpdate("Delete  from cheque where req_id='"+s[i]+"'");

         }

             PreparedStatement pst=con.prepareStatement("select * from cheque");
             ResultSet rs=pst.executeQuery();
             while(rs.next())
             {
                 ChqBean mb=new ChqBean();
                 mb.setAcno(rs.getString(1));
                 mb.setReq_id(rs.getString(2));
                 mb.setReq_date(rs.getString(3));
